...
## 표의 비교 항목(행)은 MECE + 흐름 순서여야 한다

**증상**: 초기 버전은 `table_rows`에 "가장 판단에 중요한 항목들"이라고만
지시했다 — 순서나 항목 간 중복 방지에 대한 지침이 없었다. 그 결과 항목들이
임의 순서로 나열되거나, 서로 다른 각도에서 같은 내용을 반복하는 행이 나올
위험이 있었다.

**해결**: `GENERATE_PROMPT`의 `table_rows` 필드와 `SYSTEM_PROMPT`에 두 가지를
명시적으로 요구한다: (1) 항목들이 **MECE**(상호 배타적·전체 포괄 — 두 행이
같은 내용을 다른 각도로 반복하지 않고, 이 비교에서 중요한 축을 빠뜨리지
않음)일 것, (2) 비교 대상 고유의 **자연스러운 흐름**(예: 요청/데이터가 실제로
거치는 순서 — 연결 수립 → 데이터 전송 → 순서 보장 → 오류 처리 → 사용 사례,
또는 생성 → 사용 → 해제 같은 생애주기)을 따를 것 — 임의 순서나 중요도순
나열 금지. "TCP vs UDP" 실제 테스트에서 연결 수립 → 전달 보장 → 순서 →
흐름/혼잡 제어 → 오류 처리 → 오버헤드/지연 → 사용 사례 순으로, 각 행이
겹치지 않는 결과를 확인했다.

## 핵심 키워드 볼드+파란색 강조 — 표가 아니라 Key Differences에만

**시행착오**: (1) 처음에는 표 셀 + Key Differences 둘 다에 "가장 결정적인
키워드 하나를 `<strong class="kw">`로 감싸라"고 지시했는데, 모델이 셀 전체
문장을 통째로 감싸버렸다("Multiple transactions proceed in parallel" 전체가
볼드). (2) "1-3 단어, 절대 전체 절 금지"로 프롬프트를 좁혀서 실제로 짧은
용어만 강조되는 것까지 확인했다("SELECT FOR UPDATE", "version column" 등).
(3) 하지만 실제 배포 후 사용자가 "표에 파란색이 너무 많아서 보기 불편하다"고
피드백 — 표는 이미 8행 × 2열, 즉 최대 16개까지 강조색이 들어갈 수 있어
시각적으로 과했다.

**최종 상태**: `table_rows`에서는 키워드 강조 지침을 완전히 제거했다(표는
일반 텍스트만). 대신 다음 세 곳에서만 항목당(또는 문장당) 키워드 하나를
`<strong class="kw">`로 강조한다 — 전부 개수가 적어서(2~5개) 과하지 않다:

- `summary`(Overview): 비교 대상 **각각**에서 하나씩, 총 2개
- `key_differences`(최대 3-5개 불릿): 불릿당 하나
- `when_to_use_left` / `when_to_use_right`: 문장당 하나(결국 좌/우 하나씩)

새 도메인 적용 시에도 표에는 강조를 넣지 말 것. `assets/css/extended/compare.css`의
`.post-content strong.kw { color: var(--compare-a); }` 규칙 자체는 표에도
여전히 적용되므로, 프롬프트에서 표 쪽 지침을 다시 넣으면 이 문제가 재발한다.
"Horizontal Scaling vs Vertical Scaling" 실전 테스트로 Overview 2개 +
When to Use Each 2개(문장당 1개) + 표 0개가 정확히 지켜지는 것을 확인했다.

## SVG 도식 안에 빈 줄이 있으면 렌더링이 깨진다

**증상**: 프로덕션에서 3개 포스트(Process vs Thread, DNS vs DHCP, Star Schema
vs Snowflake Schema)의 "Comparison Diagram"이 브라우저에서 비어 보인다는
사용자 리포트. 마크다운 원본을 열어보면 `diagram_svg` 자체는 멀쩡한 SVG
마크업을 담고 있었다 — 다만 대부분의 도식과 달리 사람이 읽기 좋게 들여쓰기된
**여러 줄** 형태였고, 그 줄들 사이에 **빈 줄**이 섞여 있었다.

**원인**: `<div class="compare-diagram">...</div>` 블록은 CommonMark/goldmark
기준으로 "HTML 블록"으로 파싱되는데, 이 블록은 **빈 줄을 만나면 그 지점에서
끝난다**. 빈 줄 다음에 오는 내용(예: 다음 `<text>` 태그)은 더 이상 raw HTML로
취급되지 않고 일반 마크다운 문단으로 재해석되어 `<p>` 태그로 감싸진다.
그 결과 실제 렌더링된 HTML은 `<svg>...<p><text>...</text></p><rect>...
</svg></p>`처럼 `</svg>` 종료 태그가 엉뚱하게 삽입된 `<p>` 안에 갇히는
기형적 구조가 되고, 브라우저는 이를 파싱하다가 도식 전체를 표시하지 못한다.
한 줄로 촘촘하게 이어붙인 SVG(대부분의 생성 결과)는 애초에 내부에 줄바꿈이
없으므로 이 문제에 걸리지 않는다 — 모델이 가끔 가독성을 위해 들여쓰기된
멀티라인 SVG를 생성할 때만 재현된다.

**해결**: `parse_result()`가 `diagram_svg`를 정리하는 단계에서 빈 줄을
무조건 제거한다(`"\n".join(line for line in svg.split("\n") if line.strip())`).
이미 한 줄짜리인 SVG에는 아무 영향이 없고, 멀티라인 SVG의 경우 태그 사이
줄바꿈 자체는 유지한 채 빈 줄만 없애 HTML 블록이 끊기지 않게 만든다 —
굳이 전체를 한 줄로 압축할 필요는 없다. 새 도메인에 이식할 때도 이 정리
로직을 절대 제거하지 말 것. 이미 게시된 포스트에서 같은 증상이 재발하면
`content/posts/*.md`를 스캔해 `<div class="compare-diagram">`와 `</div>`
사이에 빈 줄(`\n\n`)이 있는 파일을 찾아 빈 줄만 제거하면 된다(내용 자체는
멀쩡하므로 재생성 불필요).
