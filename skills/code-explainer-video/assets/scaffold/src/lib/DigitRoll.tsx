// origin: video-shotcraft assets/lib/DigitRoll.tsx — odometer digit column roll.
...
export const DigitRoll: React.FC<{
  ...
  fontSize?: number;
  color?: string;
}> = ({ value, delay = 0, fontSize = 30, color = T.ok }) => {
  ...
  return (
    ...
    >
      {value.split('').map((ch, i) => {
        ...
        if (target < 0) {
          return (
            <span key={i} style={{ fontSize, lineHeight: `${lineH}px`, color }}>
              {ch}
            </span>
          );
        }
