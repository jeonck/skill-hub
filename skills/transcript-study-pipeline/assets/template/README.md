...
영어 회화 수업({{SITE_TITLE}})에서 수집한 스크립트를 붙여넣으면, Claude가 분석해
...
input/script.md (수업 스크립트 전체를 코드블록에 붙여넣기, GitHub 웹 UI에서 수정)
        │
        ▼  저장(커밋)하는 순간 push 후킹으로 즉시 실행 + 매일 {{CRON_KST_TIME}} KST 크론
...
1. GitHub 저장소에서 [`input/script.md`](input/script.md) 파일을 연다.
   (블로그 상단 "{{INPUT_BUTTON_LABEL}}" 버튼으로 바로 이동 가능)
...
않는다. Actions 탭 → "{{WORKFLOW_NAME}}" → "Run workflow"로 수동 실행도 가능하다.
...
코드블록을 비워두면 매일 {{CRON_KST_TIME}} KST 크론이 `pipeline/generate.py`의
...
| `.github/workflows/daily.yml` | push 후킹 + 매일 {{CRON_KST_TIME}} KST 생성/배포 워크플로 |
