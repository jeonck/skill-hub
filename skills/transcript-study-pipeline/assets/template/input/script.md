<!--
  Paste your English class transcript inside the code block below.
  - Edit this file right on GitHub (repo page → this file → pencil ✏️ icon → edit → Commit changes).
    No local git work needed. The "{{INPUT_BUTTON_LABEL}}" button on the blog links here.
  - The entire code block is treated as ONE transcript → one study post
  ...
  - Saving (committing) this file triggers the pipeline immediately (hook on push),
    and it also runs daily at {{CRON_KST_TIME}} KST. A transcript that was already published is
